...
module.exports = function (ret, conf, settings, opt) { //打包后处理
    ...
    function getDepList(file, added) {
        ...
        file.requires.forEach(function (depId) {
            //过滤已经引入的模块和无需自动引入的模块
            if (added[depId] || (settings.excludeModule && settings.excludeModule.indexOf(depId) >= 0)) {
                return false;
            }
            added[depId] = true;
    ...
    function getAsyncList(file, added, depScaned) {
        ...
        file.requires.forEach(function (depId) {
            //过滤已经引入的模块和无需自动引入的模块
            if (depScaned[depId] || (settings.excludeModule && settings.excludeModule.indexOf(depId) >= 0)) {
                return false;
            }
